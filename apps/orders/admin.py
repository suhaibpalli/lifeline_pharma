...
class OrderAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ["order_number", "created_at", "updated_at", "delivery_address"]
    ...
    fieldsets = (
        ...
        ("Payment", {"fields": ("payment_method", "payment_id", "razorpay_order_id")}),
        (
            "Delivery",
            {
                "fields": (
        ...
        (
            "Admin Fields",
            {"fields": ("processed_by", "processed_at"), "classes": ("collapse",)},
        ),
        (
            "Timestamps",
    ...
    def mark_confirmed(self, request, queryset):
        updated = queryset.filter(status="PENDING").update(
            status="CONFIRMED", processed_by=request.user, processed_at=timezone.now()
        )
    ...
    def mark_shipped(self, request, queryset):
        updated = queryset.filter(status__in=["CONFIRMED", "PROCESSING"]).update(
            status="SHIPPED", processed_by=request.user, processed_at=timezone.now()
        )
    ...
    def mark_delivered(self, request, queryset):
        updated = queryset.filter(status="OUT_FOR_DELIVERY").update(
            status="DELIVERED", actual_delivery=timezone.now()
        )
...
class CouponAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Validity", {"fields": ("start_date", "end_date", "is_active")}),
        (
            "Timestamps",
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
        ),
